...
def extract_cabin_letter(df, var):
    # function captures the first letter
    return df[var].str[0]
...
def add_missing_indicator(df, var):
    ...
    print(f'imputing {var} with median value: {median_val}')
...
def encode_categorical(df, var):
    ...
    print (f"dropping var: {var}")
    df.drop(labels=var, axis=1, inplace=True)
    print (f"shape after encoding categoricals: {df.shape}")
...
def check_dummy_variables(df, dummy_list):
    ...
    l = [x for x in df.columns if x in dummy_list and x not in ['pclass', 'age', 'sibsp', 'parch', 'fare']]
    print (l)
    missing_var = (list(set(config.DUMMY_VARIABLES) - set(l))) 
    print (missing_var)
    if len(missing_var) >= 1:
        missing_var = "".join(missing_var)
        df[missing_var] = 0
        return df
    else:
        return df
...
def scale_features(df, output_path):
    ...
    scaler = None    
    scaler = joblib.load(scaler, output_path)
